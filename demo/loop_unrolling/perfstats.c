...
//    prefetch_misses = readcounter(DL1_PREFETCH_MISSES);
    ...
    fprintf(stderr, "%llu,%llu,%lf,%lf,%lf,%lf,%llu,%llu,%llu,%llu\n",  
                     ic, 
                     cycles,
                     (double)cycles/ic,
                     ((double)elapsed_time*1000000000.0)/cycles, 
                      elapsed_time,(double)(load_misses+store_misses+prefetch_misses)/(double)(loads+stores+prefetches),load_misses+store_misses+prefetch_misses,loads+stores+prefetches,branches,branch_misses);
